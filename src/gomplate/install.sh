...
if [ "${GOMPLATE_VERSION}" != "none" ] && ! type gomplate > /dev/null 2>&1; then

    echo "Downloading gomplate..."

    find_version_from_git_tags GOMPLATE_VERSION https://github.com/hairyhenderson/gomplate
    ...
    cp /tmp/gomplate_linux-${architecture} /usr/local/bin/gomplate
    chmod 0755 /usr/local/bin/gomplate
    rm /tmp/gomplate_linux-${architecture}
    if ! type gomplate > /dev/null 2>&1; then
        echo '(!) gomplate installation failed!'
        exit 1
    fi
else
    if ! type gomplate > /dev/null 2>&1; then
        echo "Skipping gomplate."
    else
        echo "gomplate already instaled"
    fi
fi
